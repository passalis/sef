...
def dataset_loader(dataset='mnist', seed=1):
    """
    ...
    if dataset == 'mnist':
        ...
        data, labels = load_15_scene_bow_features()
        ...
        data, labels = load_corel_bow_features()
        idx = np.random.permutation(data.shape[0])
        data = data[idx]
        labels = labels[idx]
        ...
        train_data, train_labels, test_data, test_labels = load_kth()
...
def load_15_scene_bow_features():
    """
...
def load_yale_dataset():
    ...
    with open('data/yale.pickle', 'rb') as f:
